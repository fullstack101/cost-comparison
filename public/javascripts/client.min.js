...
httpJSONRequest("http://cost-comparison.azurewebsites.net/testIP").then(json => document.getElementById("col1").appendChild(document.createElement('pre')).innerHTML = syntaxHighlight(json));
...
httpJSONRequest("http://cost-comparison.azurewebsites.net/numbeo").then(function (json) {
    ...
    let x = d3.scaleLinear().domain([0, d3.max(json[0].prices, obj => obj.average_price)]).range([0, width]);

    let xText = d3.scaleLinear().domain([0, d3.max(json[0].prices, obj => obj.item_name.length)]).range([0, width]);

    let chart = d3.select(".chart").attr("width", width + margin.left + margin.right).attr("height", (barHeight + 1) * json[0].prices.length + margin.top + margin.bottom).append("g").attr("transform", "translate(" + 0 + "," + margin.top + ")");
    ...
    let bar = chart.selectAll("g").data(json[0].prices).enter().append("g").attr("transform", function (d, i) {
