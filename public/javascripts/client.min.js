...
httpJSONRequest("/testIP").then(json => console.log(json));

httpJSONRequest("/numbeo").then(function (json) {
...
function drawLeftChart(cityJSON, col, chartClass) {
    ...
    let chart = d3.select(chartClass).attr("width", width + margin.left + margin.right - 1).attr("height", (barHeight + 1) * cityStats.length + margin.top + margin.bottom).append("g").attr("transform", "translate(" + 0 + "," + margin.top + ")");

    let bar = chart.selectAll("g").data(cityStats).enter().append("g").attr("transform", function (d, i) {
        return "translate(0," + (i * (barHeight + 1) + 3) + ")";
    });

    let xAxis = chart.append("g").attr("class", "axis axis--x").call(d3.axisTop(x).ticks(10, "$"));
    xAxis.selectAll("text").attr("transform", "matrix(-1 0 0 1 0 0)").style("text-anchor", "middle");
...
function drawRightChart(cityJSON, col, chartClass) {
    ...
    let xText = d3.scaleLinear().domain([0, d3.max(cityStats, obj => obj.item_name.substring(0, obj.item_name.indexOf(",")).length + 6)]).range([0, width]);
    ...
    let xAxis = chart.append("g").attr("class", "axis axis--x").call(d3.axisTop(x).ticks(10, "$")).attr("transform", "translate(" + 0 + ", 0)");
    xAxis.selectAll("text").style("text-anchor", "middle");
    xAxis.selectAll("text").append("text").attr("x", 6).attr("dx", "0.71em").attr("text-anchor", "end").text("Price").attr("transform", "translate(" + 20 + ", -9)");

    bar.append("rect").attr("width", function (d) {
        return x(d.average_price);
    }).attr("height", barHeight).attr("class", "bar");

    bar.append("text").attr("x", 10).attr("y", barHeight / 2).attr("dy", ".35em").style("text-anchor", "start").text(function (d) {
        let itemName = d.item_name.substring(0, d.item_name.indexOf(","));
